...
pp.parseObjjInterface = function(node) {
  ...
  if (this.eat(tt.colon))
    ...
    node.categoryname = this.parseIdent(true);
    this.expect(_parenR, "Expected closing ')' after category name");
  }
  if (this.value === '<') {
    this.next();
    var protocols = [],
    ...
    while (tokVal !== '>') {
      if (!first)
        this.expect(_comma, "Expected ',' between protocol names");
      else first = false;
      protocols.push(this.parseIdent(true));
    }
    this.next();
  ...
  node.body = [];
  while(!this.eat(oatt._end)) {
    if (tokType === _eof) this.raise(tokPos, "Expected '@end' after '@interface'");
...
pp.parseObjjIvarDeclaration = function(node) {
  ...
  for (;;) {
    ...
    if (this.strict && isStrictBadIdWord(decl.id.name))
...
pp.parseObjjClassElement = function() {
  ...
  if (this.value === '+' || this.value === '-') {
    ...
    this.objjInFunction = true; this.objjLabels = [], this.objjFunctionIsAsync = !!element.returntype && !!element.returntype.async;
